...
Trigger: any change under `Sources/`, `App/`, `Package.swift`, `build.sh`, or anything else that affects the shipped `.app` binary.
...
3. Run:
   ...
   ```
   This cleans `.build/`, runs `build.sh`, zips `vol-mixer.app` with `ditto` (codesign-preserving), tags, pushes the tag, and creates the GitHub release with the zip attached.
4. Confirm:
   ```sh
